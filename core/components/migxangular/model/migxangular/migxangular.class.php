...
class MigxAngular {
    ...
    /**
     * @access public
     * @var array A collection of properties to adjust MIGXangular behaviour.
     */
    public $config = array();
    /**
     * @access public
    ...
    /**
     * The MIGXangular Constructor.
     *
     * This method is used to create a new MIGXangular object.
     *
     * @param modX &$modx A reference to the modX object.
     * @param array $config A collection of properties that modify MIGXangular
     * behaviour.
     * @return MIGXangular A unique MIGXangular instance.
     */
    function __construct(modX & $modx, array $config = array()) {
        ...
        */
        $corePath = $this->modx->getOption('migxangular.core_path', null, $modx->getOption('core_path') . 'components/migxangular/');
        $assetsPath = $this->modx->getOption('migxangular.assets_path', null, $modx->getOption('assets_path') . 'components/migxangular/');
        $assetsUrl = $this->modx->getOption('migxangular.assets_url', null, $modx->getOption('assets_url') . 'components/migxangular/');
        $migxCorePath = $this->modx->getOption('migx.core_path', null, $modx->getOption('core_path') . 'components/migx/');
        $migxAssetsUrl = $this->modx->getOption('migx.assets_url', null, $modx->getOption('assets_url') . 'components/migx/');

        $defaultconfig['migxCorePath'] = $migxCorePath;
        ...
        $defaultconfig['migxConnectorUrl'] = $migxAssetsUrl . 'connector.php';
        ...
        if (!empty($packageName)) {
            ...
            switch ($type) {
                case 'processors':
                    ...
                    break;
                case 'grids':
                    $path = $packagepath . 'migxtemplates/web/grids/';
    ...
    public function prepareGrid($properties, &$controller, &$tv, $columns = array()) {
        ...
        $controller->setPlaceholder('update_win_title', !empty($this->customconfigs['update_win_title']) ? $this->customconfigs['update_win_title'] : 'MIGXangular');
     ...
     * @return string
     */
    public function getTvRender(&$tv, $params, $value, array $paths, $method, $resourceId = 0, $type = 'text') {
        if (empty($type))
            $type = 'text';
        ...
        if ($className = $tv->checkForRegisteredRenderMethod($type, $method)) {
            ...
            $render = new $deprecatedClassName($tv);
            ...
            if (empty($output)) {
                $p = $this->modx->getOption('processors_path') . 'element/tv/renders/mgr/' . $method . '/';
